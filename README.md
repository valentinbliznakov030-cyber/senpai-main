...
## ⚠️ ВАЖНО: Външни API-та (Задължителни!)

**БЕЗ ТЕЗИ API-ТА САЙТЪТ НЕ РАБОТИ!** Тези API-та са критични за функционирането на платформата и трябва да бъдат стартирани преди `senpai-main`.

### 1. hianime-API

**Какво е:**
- RESTful API за извличане на аниме данни от hianime.to
- Използва се за търсене, списъци, епизоди и стрийминг линкове

**Изисквания:**
- **Bun.js** (не Node.js!) - задължително
- Bun е бърз JavaScript runtime, подобен на Node.js

**Инсталация на Bun.js:**

**Windows:**
```powershell
# Изтегли и инсталирай от: https://bun.sh/docs/installation
# Или чрез PowerShell:
...
**Стартиране на hianime-API:**
```bash
# Отиди в папката, където си изтеглил hianime-API
cd <папка-на-hianime-API>
bun install
bun run dev
# API ще работи на http://localhost:3030
```
...
### 2. api.consumet.org (Consumet API)

**Какво е:**
- API за достъп до информация за аниме, филми, книги и др.
- Използва се като алтернативен източник на данни
...
**Стартиране на Consumet API:**
```bash
# Отиди в папката, където си изтеглил api.consumet.org
cd <папка-на-api.consumet.org>
npm install
npm run dev
# API ще работи на http://localhost:3000 (или друг порт според конфигурацията)
```

**Порт:** 3000 (по подразбиране)

---

### ⚡ Бърз старт на всички API-та
...
5. **След това стартирай останалите сервиси** (MySQL, Redis, LibreTranslate, anime-service, subscription-service)

**Важно:** Тези API-та трябва да работят преди да стартираш `senpai-main`!
...
- **Bun.js** (за hianime-API) - **ЗАДЪЛЖИТЕЛНО!**
- **Node.js 18+** (за api.consumet.org) - **ЗАДЪЛЖИТЕЛНО!**

### Инсталация на Java 17

#### Windows (Препоръчително - чрез скрипт)

Използвайте предоставения PowerShell скрипт за автоматична инсталация:
...
- Проверява дали Java е вече инсталирана
- Инсталира Eclipse Temurin 17 (JDK) чрез winget
- Приема лицензите автоматично (без интерактивни подкани)
- Обновява PATH променливата

#### Windows (Ръчна инсталация чрез winget)

Ако предпочитате ръчна инсталация:

```powershell
# Важно: Добавете флаговете за автоматично приемане на лицензите!
winget install "Eclipse Adoptium.Temurin.17.JDK" --accept-source-agreements --accept-package-agreements --silent
```

**Забележка:** Без флаговете `--accept-source-agreements` и `--accept-package-agreements`, winget ще поиска интерактивно потвърждение, което може да причини грешки при автоматизация.

#### Windows (Алтернативен метод)

Ако winget не работи, изтеглете Java ръчно:
1. Отидете на: https://adoptium.net/temurin/releases/?version=17
2. Изтеглете JDK 17 за Windows (x64)
3. Инсталирайте и добавете в PATH
...
След инсталация, проверете:

```bash
java -version
# Трябва да покаже версия 17 или по-нова
```
...
**ВАЖНО:** Всички сервиси трябва да са стартирани в следния ред:

1. **Стартирай hianime-API (ЗАДЪЛЖИТЕЛНО!):**
   ```bash
   # Отиди в папката, където си изтеглил hianime-API
   cd <папка-на-hianime-API>
   bun install
   bun run dev
   # API работи на http://localhost:3030
   ```

2. **Стартирай Consumet API (ЗАДЪЛЖИТЕЛНО!):**
   ```bash
   # Отиди в папката, където си изтеглил api.consumet.org
   cd <папка-на-api.consumet.org>
   npm install
   npm run dev
   # API работи на http://localhost:3000
   ```
...
5. **Стартирай LibreTranslate (ако е нужно):**
   ```bash
   docker start libretranslate
   ```

6. **Стартирай anime-service и subscription-service:**
   - Тези сервиси също трябва да са стартирани
...
**Какво прави:**
- Извличане на аниме данни от hianime.to
- Търсене на аниме
- Списъци (trending, popular, latest, etc.)
- Детайли за аниме
- Епизоди и стрийминг линкове

**Изисквания:**
- **Bun.js** инсталиран (НЕ Node.js!)
- Стартиран преди `senpai-main`
- Виж секцията [ВАЖНО: Външни API-та](#-важно-външни-api-та-задължителни) по-горе за инсталация
...
**Важно:** Без този API сайтът **НЕ РАБОТИ**!

---

### 2. Consumet API (api.consumet.org) (ЗАДЪЛЖИТЕЛНО!)

**URL:** `http://localhost:3000/` (или друг порт според конфигурацията)

**Порт:** 3000 (по подразбиране)

**Какво прави:**
- Алтернативен източник на аниме данни
- API за достъп до информация за аниме, филми, книги
- Използва се като резервен/допълнителен източник

**Изисквания:**
- **Node.js 18+** инсталиран (НЕ Bun!)
- Стартиран преди `senpai-main`
- Виж секцията [ВАЖНО: Външни API-та](#-важно-външни-api-та-задължителни) по-горе за инсталация
...
### 4. Subscription Service (Feign Client)
...
### 6. LibreTranslate (Translation Service)
...
| hianime-API | 3030 | HTTP | http://localhost:3030 | - |
| Consumet API | 3000 | HTTP | http://localhost:3000 | - |
...
- **hianime-API** и **Consumet API** са **ЗАДЪЛЖИТЕЛНИ** - без тях сайтът **НЕ РАБОТИ**!
- **hianime-API** изисква **Bun.js** (НЕ Node.js!)
- **Consumet API** изисква **Node.js 18+** (НЕ Bun!)
